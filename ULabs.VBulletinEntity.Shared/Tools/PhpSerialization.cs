...
namespace ULabs.LightVBulletinEntity.Tools {
    // Source: https://github.com/Anheledir/anheledir.NET/blob/master/_posts/2011-05-04-php-un-serialize-with-c.md
    public class PhpSerialization {
        ...
        public string Serialize(object obj) {
            ...
            return this.DoSerialize(obj, new StringBuilder()).ToString();
        }//Serialize(object obj)

        private StringBuilder DoSerialize(object obj, StringBuilder sb) {
            if (obj == null) {
                return sb.Append("N;");
                ...
                // StringEncoding.GetByteCount returns +1 offset instead of the real string length, so we modified it to make it vBulletin (PHP) compatible
                ...
                for (int i = 0; i < a.Count; i++) {
                    this.DoSerialize(i, sb);
                    this.DoSerialize(a[i], sb);
                }
                sb.Append("}");
                return sb;
                ...
                foreach (DictionaryEntry entry in a) {
                    this.DoSerialize(entry.Key, sb);
                    this.DoSerialize(entry.Value, sb);
                }
                sb.Append("}");
                return sb;
            } else {
            ...
            switch (str[this.pos]) {
                ...
                case 'a':
                    ...
                    for (int i = 0; i < length; i++) {
                        //read key
                        object key = DoDeserialize(str);
                        //read value
                        object val = DoDeserialize(str);
