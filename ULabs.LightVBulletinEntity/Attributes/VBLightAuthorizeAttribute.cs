        ...
        public void OnResourceExecuting(ResourceExecutingContext context) {
            ...
            if (!userSession.IsLoggedIn) {
                if (string.IsNullOrEmpty(loginRedirectUrl)) {
                    // ToDo: Replace by light settings manager if exists
                    var db = GetService<MySqlConnection>(context);
                    string sql = "SELECT value FROM setting WHERE varname = 'bburl'";
                    loginRedirectUrl = db.Query<string>(sql).FirstOrDefault();
